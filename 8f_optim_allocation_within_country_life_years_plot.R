...
df1 <- read_csv("optim_dataframes/fig_4_inputs_life_years.csv")
...
g2 <- plot3(filter(df1, income_group == "HIC"))
g4 <- plot3(filter(df1, income_group == "UMIC"))
g6 <- plot3(filter(df1, income_group == "LMIC"))
g8 <- plot3(filter(df1, income_group == "LIC"))
...
all_plots <- (g1 | g2 | g3 | g4 | g5 | g6 | g7 | g8) + plot_layout(guide = "collect", ncol = 2, nrow = 4, heights = c(1, 1, 1, 1))+ plot_annotation(tag_levels = 'A') +
  plot_annotation(title = paste0("Sensitivity analysis: life-years gained as outcome measure"))

all_plots

ggsave("plots/Fig4_life_years.png", all_plots, height = 11, width = 10)
